...
export class SingaporeWeatherClient {
  ...
  async getCurrentWeather(latitude: number, longitude: number): Promise<WeatherSnapshot> {
    const settle = async <T>(fn: () => Promise<T>, delayMs: number) => {
    ...
    // Stagger concurrent execution to avoid api-open.data.gov.sg rate limits (HTTP 429)
    // while significantly improving overall execution speed compared to sequential fetches.
    const [
      forecastPayload,
      airTemp,
      relativeHumidity,
