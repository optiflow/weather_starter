...
export class SingaporeWeatherClient {
  ...
  async getCurrentWeather(latitude: number, longitude: number): Promise<WeatherSnapshot> {
    // ⚡ Bolt: Execute external API calls concurrently with staggered delays.
    // Instead of sequentially waiting + injecting a 1s delay per call (~20s total),
    // we stagger the requests in a Promise.all block. This maximizes throughput
    // and cuts response time down to ~2s while still avoiding HTTP 429 rate limit errors
    // from api-open.data.gov.sg.
    const settle = async <T>(thunk: () => Promise<T>, delayMs: number) => {
