...
export class SingaporeWeatherClient {
  ...
  async getCurrentWeather(latitude: number, longitude: number): Promise<WeatherSnapshot> {
    const settle = async <T>(promiseFn: () => Promise<T>, delay: number) => {
      // Staggering execution slightly to avoid HTTP 429 Rate Limit from api-open.data.gov.sg
      // This is a performance optimization: previous implementation awaited sequentially,
      // now we run concurrently but staggered.
      if (delay > 0) {
