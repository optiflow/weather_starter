...
export class SingaporeWeatherClient {
  ...
  async getCurrentWeather(latitude: number, longitude: number): Promise<WeatherSnapshot> {
    // ⚡ Bolt: Batching requests with staggered starts.
    // Staggering prevents HTTP 429 from strict rate limits while reducing
    // total wait time from ~28 seconds (sequential 1000ms waits) to ~4.5 seconds.
    const settle = async <T>(promiseFactory: () => Promise<T>, delayMs: number) => {
