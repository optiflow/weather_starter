...
export class SingaporeWeatherClient {
  ...
  async getCurrentWeather(latitude: number, longitude: number): Promise<WeatherSnapshot> {
    ...
    // Replaced sequential awaits (which took >10s) with staggered concurrent execution.
    // By using `Promise.all` with staggered start times, we maximize performance while
    // avoiding HTTP 429 rate limits from api-open.data.gov.sg.
